...
export default function CreateSpot() {
    ...
    const inputs = [
        { name: 'country', placeholder: 'Country', type: 'text', label: "true" },
        { name: 'address', placeholder: 'Street Address', type: 'text', label: "true" },
        ...
        { name: 'name', placeholder: 'Name of your spot', type: 'text', label: "true" },
    ...
    const handleSubmit = async (e) => {
        ...
        Object.keys(formData).forEach((key) => {
            if (!formData[key].length) {
                if (key === "description") {
                    ...
                    fieldCheck[key] = `${key[0].toUpperCase()}${key.slice(1)} is required`;
                }
            }
        });
        ...
        const payload = {};
        Object.entries(formData).forEach(([key, value]) => { // Corrected this line
            ...
            <form className={styles.createSpotForm} onSubmit={handleSubmit}>
                    ...
                    {inputs.map((input) => (
                        <div key={input.name} className={styles.singleInput}>
                            {input.label === "true" && <label htmlFor={input.name}>{input.placeholder}</label>}
                            {input.type !== 'textarea' ? (
                                <input
